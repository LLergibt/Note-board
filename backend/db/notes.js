...
export const deleteProperty = async (propertyId) => {
   ...
   await sequelize.query(`DELETE FROM property_note WHERE property_id=${propertyId}`)
   await sequelize.query(`DELETE FROM property WHERE id=${propertyId}`)
}
...
export const selectChoice = async (body) => {
  await sequelize.query(`UPDATE choose_property_note SET choose_id=${body.choose_id} WHERE property_note_id=${body.property_note_id} RETURNING *`)
  await propertyDataChange({data: body.title, id: body.property_note_id})
}
...
export const choosePropertyCreate = async (body) => {
  ...
  await propertyDataChange({data: body.title, id: body.property_note_id})
  //await sequelize.query(`update property_note set data='${body.data}' where id=${body.id}`)
...
export const propertyToggle = async (body) => {
  console.log(body)
  await sequelize.query(`UPDATE property SET is_toggled='${body.is_toggled}' WHERE id=${body.id}`)

}


export const propertyTypeChange = async(body) => {
  await sequelize.query(`UPDATE property SET type_id=${body.type_id} WHERE id=${body.id}`)
  await sequelize.query(`update property_note set data=${null} where property_id=${body.id}`)
}
...
export const propertiesGet = async (boardId) => {
  const [properties, metadata] = await sequelize.query(`SELECT pn.id, pn.data, p.title, note_id, pn.property_id, p.is_toggled, types.title as types_title, types.id as types, types.category as types_category FROM property_note pn LEFT JOIN property p ON p.id = pn.property_id left join types ON types.id = p.type_id ORDER BY pn.id`)
  return properties
}
